...
export default function ProductAccordion({ product }: { product: any }) {
  return (
    <div className="mt-4 border-t-8 border-black">
      ...
      {/* 1. DESCRIPTION SECTION (Renamed from The Story) */}
      <AccordionItem title="Description" icon={<FileText size={20} />} defaultOpen={true}>
        <div className="bg-gray-50 border-l-8 border-jungli-orange p-6 shadow-brutal-sm">
          {/* whitespace-pre-line is the key fix: it respects your line breaks and paras */}
      ...
      <AccordionItem title="Build Reports" icon={<Zap size={20} />}>
            ...
            <ul className="space-y-3">
               {(product.bullet_points || [
                 ...
                 <li key={i} className="flex items-start gap-3">
                   <span className="text-jungli-orange font-black text-xs">⚡️</span>
        ...
        {/* Branded Unboxing Section */}
        <div className="mt-6 bg-jungli-green text-white p-6 border-4 border-black shadow-brutal-sm relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-2 opacity-10 rotate-12"><PackageCheck size={100}/></div>
            <div className="relative z-10">
                <div className="flex items-center gap-3 mb-3 border-b-2 border-white/20 pb-2">
                  <PackageCheck size={24} className="text-yellow-400" />
                  <p className="font-[1000] italic text-lg uppercase tracking-tighter leading-none">THE UNBOXING STASH</p>
                </div>
                <p className="text-[10px] font-bold italic opacity-90 leading-relaxed uppercase tracking-widest">
                   Secured in our signature <span className="text-yellow-400 font-black">MATTE BLACK BOX</span>. 
                   Includes dust bags, authenticity cards, and extra laces.
                </p>
            </div>
        </div>
      </AccordionItem>
      ...
      <AccordionItem title="Size Guide" icon={<Ruler size={20} />}>
        <div className="bg-[#FFFFEE] border-4 border-black p-4 md:p-6 shadow-brutal relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h4 className="text-2xl font-[1000] uppercase italic text-center mb-6 tracking-tighter text-black">
                MEN'S SIZE CHART
            </h4>
            ...
            <div className="overflow-x-auto border-4 border-black shadow-brutal-sm bg-white">
              <table className="w-full text-center border-collapse min-w-[350px]">
                <thead>
                  <tr className="border-b-4 border-black bg-gray-50 uppercase text-[9px] font-black tracking-[0.2em]">
                    <th className="py-3 px-2">Foot Length (CM)</th>
                    <th className="py-3 px-2 text-white bg-black border-x-4 border-black">UK / IND</th>
                    <th className="py-3 px-2 text-gray-400">EURO</th>
                  </tr>
                </thead>
                <tbody className="font-black uppercase italic text-base">
                  {[
                    ...
                    <tr key={i} className="border-b-2 border-gray-100 hover:bg-yellow-100 transition-colors">
                      <td className="py-3 text-xs">{row.cm}</td>
                      <td className="py-3 text-xl font-[1000] text-black border-x-4 border-black bg-yellow-400/20">{row.uk}</td>
                      <td className="py-3 text-xs text-gray-300">{row.eu}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="mt-6 flex items-center gap-3 bg-white border-2 border-black border-dashed p-3">
                <Footprints size={20} className="text-jungli-orange" />
                <p className="text-[9px] font-bold uppercase italic text-gray-600">
                    Mark heel to toe on paper. Dist CM = Size.
                </p>
            </div>
          </div>
        </div>
