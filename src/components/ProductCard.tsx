...
export default function ProductCard({ 
  ...
  // 1. DYNAMIC VIEWER COUNT LOGIC
  const [viewers, setViewers] = useState<number | null>(null);

  useEffect(() => {
    // Generate a random number between 0 and 200
    // We do this in useEffect so it only happens on the client side
    const randomCount = Math.floor(Math.random() * 201);
    setViewers(randomCount);
  }, []);
  ...
  return (
    <Link href={`/shop/${id}`} className="block h-full group"> 
      <motion.div 
        ...
        viewport={{ once: true }}
        className={`relative bg-white border-4 border-black p-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all flex flex-col h-full overflow-hidden ${!is_available ? 'opacity-90 grayscale-[0.3]' : ''}`}
      >
        {/* TOP LABELS */}
        ...
        <div className="relative aspect-square bg-gray-50 border-2 border-black mb-4 overflow-hidden flex items-center justify-center">
          ...
          {is_available && (
            <div className="absolute bottom-2 right-2 bg-white text-black font-black px-2 py-1 text-[10px] border-2 border-black shadow-brutal-sm">
              {discount}% OFF
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          {/* 2. DYNAMIC SOCIAL PROOF: Only shows if viewers > 0 and shoe is in stock */}
          {is_available && viewers !== null && viewers > 0 && (
            <div className="flex items-center gap-1.5 text-[9px] font-black text-gray-400 uppercase italic mb-4">
              <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-ping" />
              {viewers} people looking at this
            </div>
          )}
        </div>
        ...
        <div className="mt-auto">
          <div className="flex justify-between items-end mb-4">
            ...
            <div className={`p-2 border-2 border-black shadow-brutal-sm ${is_available ? 'bg-yellow-400 text-black' : 'bg-gray-100 text-gray-400'}`}>
              {is_available ? <Zap size={20} fill="currentColor" /> : <Bell size={20} className="text-gray-400" />}
            </div>
          </div>
