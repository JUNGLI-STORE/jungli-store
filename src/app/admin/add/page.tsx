...
import { 
  ...
  Check, Video, Plus, GripVertical, Star, MessageSquare, Link2
} from "lucide-react";
...
function AddProductForm() {
  ...
  // 1. FORM STATE (Added model_group)
  const [formData, setFormData] = useState({
    name: "",
    brand: "",
    model_group: "", // NEW: To link different colors of same shoe
    luxury_price: "",
    jungli_price: "",
    tag: "NEW DROP",
    description: "",
        ...
        if (cloneId) {
          ...
          if (original) {
            setFormData({
              ...
              model_group: original.model_group || "", // Fetch existing group
  ...
  const handleLaunch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // C. Save Product (Added model_group)
      const { data: product, error: pError } = await supabase.from('products').insert([{
        ...formData,
        name: formData.name.toUpperCase(),
        brand: formData.brand.toUpperCase(),
        model_group: formData.model_group.toLowerCase().replace(/\s+/g, '-'), // Auto-format to slug
          ...
          <form onSubmit={handleLaunch} className="bg-white border-8 border-black p-6 md:p-12 shadow-brutal relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
              ...
              <div className="space-y-10">
                <div>
                  <label className="block font-black uppercase italic mb-4 text-sm tracking-widest text-jungli-orange">Texture Clips (MP4s)</label>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {videoFiles.map((v, i) => (
                      <div key={i} className="bg-black text-white px-3 py-1 border-2 border-black flex items-center gap-2 text-[10px] font-black italic">
                        <Video size={12} /> {v.name.slice(0, 10)}... <X size={12} className="cursor-pointer" onClick={() => setVideoFiles(videoFiles.filter((_, idx) => idx !== i))} />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-gray-50 border-4 border-black p-6 shadow-brutal-sm">
                  <label className="block font-black uppercase italic mb-4 text-xs tracking-widest">Available Sizes (UK)</label>
                    ...
                    {["UK 6", "UK 7", "UK 8", "UK 9", "UK 10", "UK 11"].map(size => (
                      <button key={size} type="button" onClick={() => {
                        ...
                        className={`py-3 border-4 border-black font-black italic text-sm transition-all
                          ${formData.available_sizes.includes(size) ? 'bg-jungli-orange text-white translate-x-1 shadow-none' : 'bg-white shadow-brutal-sm'}`}
                      >
                        {size}
                      </button>
                    ))}
              ...
              {/* DATA FIELDS (Updated with Model Group) */}
              <div className="space-y-6">
                ...
                <input required value={formData.name} placeholder="MODEL COLORWAY (E.G. CHICAGO HIGH)" className="w-full p-4 border-4 border-black font-[1000] uppercase italic shadow-brutal-sm" onChange={e => setFormData({ ...formData, name: e.target.value })} />

                <div className="relative group">
                  <input value={formData.model_group} placeholder="MODEL GROUP (E.G. JORDAN-1-HIGH)" className="w-full p-4 border-4 border-black font-[1000] uppercase italic shadow-brutal-sm bg-yellow-50" onChange={e => setFormData({ ...formData, model_group: e.target.value })} />
                  <div className="absolute top-[-10px] right-2 bg-black text-white text-[8px] px-2 font-black italic border-2 border-black">LINKING ID</div>
                </div>
            ...
            <div className="border-t-8 border-black pt-12 mb-16">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-4xl font-[1000] uppercase italic tracking-tighter">HUNT <span className="text-yellow-400">REPORTS</span></h2>
                <button type="button" onClick={addReviewRow} className="bg-white border-4 border-black px-6 py-2 font-black uppercase italic shadow-brutal-sm hover:translate-x-1 transition-all flex items-center gap-2 text-sm">
                ...
                {reviews.map((rev, idx) => (
                  <div key={rev.id} className="bg-gray-50 border-4 border-black p-6 grid grid-cols-1 md:grid-cols-3 gap-6 relative shadow-brutal-sm">
                    ...
                    <div className="space-y-2">
                      <input placeholder="NAME" className="w-full p-2 border-2 border-black font-black uppercase text-xs" onChange={e => { const newR = [...reviews]; newR[idx].name = e.target.value; setReviews(newR); }} />
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map(s => (
                          <Star key={s} size={20} fill={rev.rating >= s ? "black" : "none"} className="cursor-pointer" onClick={() => { const newR = [...reviews]; newR[idx].rating = s; setReviews(newR); }} />
                        ))}
                      </div>
                    </div>
                    <div className="md:col-span-2 space-y-2">
                      <textarea placeholder="CUSTOMER QUOTE..." className="w-full p-2 border-2 border-black font-bold uppercase text-[10px] italic" onChange={e => { const newR = [...reviews]; newR[idx].msg = e.target.value; setReviews(newR); }} />
                      <input type="file" className="text-[10px] font-black" onChange={e => { const newR = [...reviews]; newR[idx].file = e.target.files?.[0]; setReviews(newR); }} />
                    </div>
                  </div>
                ))}
              </div>
